...
The business model is **not** invalidated. All four providers permit commercial
use of output and none forbids building a paid application on their API.

The mechanism differs and the difference matters: OpenAI **assigns** its rights
in output to the customer, Amazon states the customer **owns** output, Adobe
**permits commercial use**, and Google **claims no ownership** without making
an affirmative grant. "Nobody claims it" is not the same as "it is yours", and
none of the four can convey rights that do not exist in the first place, which
for purely AI-generated work in the United States may be none.
...
## Condition 1: the paid Gemini tier is mandatory, not advisable

Two clauses combine here. The first, from the Gemini API Additional Terms of
Service, describes what the unpaid tier costs in privacy:
...
The second, under Use Restrictions, removes the choice:
...
Pixel Prompt is a public URL with no geo-restriction, so it makes an API client
available to users in those regions by default. **The paid tier is therefore
required, not merely advisable.** The alternatives are to pay, or to geo-block
the EEA, Switzerland, and the UK, which is a worse product for no saving.

Separately, if the key were unpaid, every prompt and every generated image
passing through Gemini would become Google training data readable by a human
reviewer. That is defensible for a hobby deployment and indefensible for a paid
product whose privacy policy says otherwise.

None of this is visible in the code. It is a property of the key's billing
status in Google Cloud, so no test can catch it and no config flag records it.

**Action:** confirm the production key is on the paid tier before launch. Treat
it as a deploy blocker rather than a checklist nicety, because serving one
EEA user on an unpaid key is a terms violation regardless of the privacy
argument.
...
Not optional and not configurable. Amazon publishes a detection API that can
check whether an image carries the watermark. AWS does not state how the
watermark survives downstream editing, cropping, or re-encoding, so treat it as
"detectable as generated by Nova", not as an indelible mark that persists
...
- Image keys contained no session id, so two sessions starting in the same UTC
  second shared a gallery folder, mixing two users' images into one gallery
  entry. At 500 generates/day the chance of at least one same-second folder
  collision was 76% on a given day. Silent **overwrite** is the narrower case
  on top of that: it also needs both sessions to use the same model and finish
  within the same second, so it is less frequent than the folder collision but
  loses data rather than merely mixing it.
