...
consumed as the check. Every path returning non-2xx after quota enforcement
calls it exactly once. **Guests are excluded deliberately**: a guest identity
is a cookie, so refunding one is an unlimited retry for anyone who wants
one.
